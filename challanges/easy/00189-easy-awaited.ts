...
// This solves the issue described
// type MyAwaited<T> = T extends Promise<infer U> ? U : T;
// but the tests requires recursiveness. This solves the recursive promise issue, but does not solves the explicit then structure
// type MyAwaited<T> = T extends Promise<infer U> ? MyAwaited<U> : T;
// This uses PromiseLike which is kind of weird. Awesome idea, have the execution
type MyAwaited<T> = T extends PromiseLike<infer U> ? MyAwaited<U> : T;

type sdg = MyAwaited<X>;
type dfgh = MyAwaited<Z>;
type dsfg = MyAwaited<T>;
